...
def evaluate_retrieval(golden_refs, model_refs):
    ...
    return np.array([golden_refs_not_in_retrieval, golden_refs_in_retrieval, retrieval_ranking_score])
...
def main(limit : int, question_type : str,):
    ...
    model2refs = {}
    model2refs['index'] = ['golden_refs_not_in_retrieval', 'golden_refs_in_retrieval', 'diff_retrieval_rank_avg']
    for line in tqdm(evaluation_file, desc="Retrieving relevant documents"):
        query = line['query']
        message = [{"role": "user", "content": query}]
        #print(f'query: {query}')
        golden_refs = [Reference('', '', line['link_to_answer_1'], line['text_snippet_1']),
                       Reference('', '', line['link_to_answer_2'], line['text_snippet_2'])]
        for model in retrievers:
            scores, refs = model.retrieve(message)
            #print(model.__class__.__name__)
            model2refs[model.__class__.__name__] = model2refs.get(model.__class__.__name__, np.array([0,0,0])) \
                + evaluate_retrieval(golden_refs, refs)
    
    df = pd.DataFrame.from_dict(model2refs)
    df.set_index('index', inplace=True)
    df.loc['diff_retrieval_rank_avg'] = (df.loc['diff_retrieval_rank_avg']/df.loc['golden_refs_in_retrieval']).round(2)
...
def df_to_table(
    ...
    for index, value_list in zip(pandas_dataframe.index.tolist(),
                                 pandas_dataframe.values.tolist()):
...
def cli():
    ...
    #TODO
    parser.add_argument('--compare-top-5', action='store_true', default=False,
