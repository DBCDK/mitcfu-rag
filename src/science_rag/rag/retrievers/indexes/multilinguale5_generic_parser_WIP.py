...
def index_paragraph_docs_GPU_batches(
    path_to_folder_with_documents: str,
    path_to_save_embeddings_and_labels: str,
    batch_size=False,
):
    """
    Indexing documents with multilingual-e5-large-instruct model using GenericParser to create a LangChain Document list,
    then embedding them in batches using e5multilingualEmbedder and storing them in a FAISS db with KNNSearch.

    Inputs:
    - path_to_folder_with_documents: str, path to the folder containing the documents to be indexed.
    - path_to_save_embeddings_and_labels: str, path where the FAISS index and labels will be stored.
    - batch_size: int or False, size of the batches for embedding.

    Outputs:
    - Saves the FAISS db (vectors) and their corresponding labels to the specified path.
    """

    # Parsing documents using GenericParser.
    generic_parser = GenericParser()
    all_files = [
        os.path.join(path_to_folder_with_documents, f)
        for f in os.listdir(path_to_folder_with_documents)
        if os.path.isfile(os.path.join(path_to_folder_with_documents, f))
    ]
    documents = generic_parser.parse_all(all_files)
    ...
    embeddings = []
    labels = []
    page_content_for_embedding_batch = []
    labels_for_batch = []
    ...
    for doc in documents:
        ...
        # before chunking, check if the text is non-string type
        if not isinstance(page_content, str):
            logger.debug(f"Page content is not a string: {page_content} in doc with source {source}")
            continue

        chunks = text_splitter.split_text(page_content)
        for i, chunk in enumerate(chunks):
            page_content_for_embedding_batch.append(chunk)
            labels_for_batch.append(f"{source}_side{page_label}_chunk{i}")

        if len(page_content_for_embedding_batch) >= batch_size:
            embeddings_for_this_batch = e5_embedder.embed_documents(page_content_for_embedding_batch)
            embeddings.extend(embeddings_for_this_batch)
            labels.extend(labels_for_batch)

            # Reset the batch for both embeddings and labels
            page_content_for_embedding_batch.clear()
            labels_for_batch.clear()

        if page_content_for_embedding_batch:
            embeddings_for_this_batch = e5_embedder.embed_documents(page_content_for_embedding_batch)
            embeddings.extend(embeddings_for_this_batch)
            labels.extend(labels_for_batch)
    ...
    logger.info(f"Saving FAISS db locally to {path_to_save_embeddings_and_labels}")
    db = KNNSearch.build(np.array(embeddings), np.array(labels))
    db.save(
        index_path=path_to_save_embeddings_and_labels + "/embeddings",
        labels_path=path_to_save_embeddings_and_labels + "/labels",
    )
    logger.info(f"FAISS db succesfully saved locally to {path_to_save_embeddings_and_labels}")
...
def main():
    ...
    index_paragraph_docs_GPU_batches(
        path_to_save_embeddings_and_labels=args.path_to_save_db,
        path_to_folder_with_documents=args.path_to_folder_with_documents,
