...
class SolrGenerator(Generator):
    ...
    def generate(self, references: list[(str, str)], messages: list[str]) -> str:
        ...
        if not references:
            return (
                "Kan jeg hjælpe dig med noget andre eller kan du finde en andre måde at beskrive hvad du søger?",
                [],
            )

        # parsed_references = [f"# {ref['article_headline']}\n## {ref['subheadline']}\n### {ref['sentences'][:175]}\nlink: {ref['article_link']}" for ref in references][:4]
        parsed_references = [
            f"{ref['sentences'][:275]}\nLink: {ref['article_link']}"
        ...
        if chat_history == "":
            chat_history = " ".join(
                [
                    msg["content"][:http_index]
                    for msg in messages[:-1]
                    if (http_index := msg["content"].find("https")) != -1
                ]
            )
