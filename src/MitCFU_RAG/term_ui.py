...
def display_query_debug(query):
    console.print(Align(Panel(f"[{input_debug_color}]"+ "\n".join(query) +f"[/{input_debug_color}]",
                              title="query", title_align='left', expand=False, style=input_debug_color), align='right', pad=True))
...
def cli():
    ...
    parser.add_argument('-s', '--no-stream', dest='stream', action='store_false', help='Dont stream output')

    args = parser.parse_args()

    print(args)
    cfg = {'display_rag_input': args.verbose,
           'stream': args.stream}
    chat(cfg)
