...
class MetaSolrRetriever(Retriever):
    ...
    def retrieve(self, messages: list[str], limit: int = 5, cli: bool = False) -> tuple[list[float], list[Reference]]:
        ...
        if len(questions)==1:
            results = solr_retrieval_tools.search_by_question(questions[0], self.searcher, n = 5)
            if cli:
        ...
        keywords = self.keyword_extractor.get_1gram_keywords(message, n = 1) \
                    + self.keyword_extractor.get_2gram_keywords(message, n = 1)
        logger.debug(f'extracted keywords: {keywords}')
        keyword_search_params = ["article_headline^50", 
                                "subheadline^50",
                                "article_topics^100",
                                "gen_keywords^50"]
        for keyword in keywords:
            results = solr_retrieval_tools.search(keyword, self.searcher, n = 5, params=keyword_search_params)
            if cli:
    ...
    def rerank(self, references: list[Reference], limit: int):
        ...
        return [float(count) for _, count in count_occurences], [ref for ref, _ in count_occurences]
