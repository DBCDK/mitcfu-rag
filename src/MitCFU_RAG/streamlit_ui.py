...
def summary_references():
    ...
    st.session_state.messages = [{"role":"assistant", "content": "Her findes en opsammling af de brugte kilder fra faktachat. \n \
                                  Svar fra Faktachat kan også indeholde information uden for de givne artikler"}] + references
    return chat_history
...
greeting = "Hej 👋, Jeg er FaktaChat og jeg kan hjælpe dig med at finde information ud af artikler\
              på Faktalink.\
              \n\nHvis du har brug for inspiration til hvad du kan spørge mig om,\
              kan du kigge på de temaer der er artikler om på Faktalink: https://faktalink.dk/tema\
...
if prompt := st.chat_input("Indsæt dit spørgmål her ..."):
    ...
    with st.chat_message("assistant", avatar="🤓"):
        fillers = ["Lad mig finde relevant information på faktalink ...", "Lad mig se...", "Et øjeblik...", "Vent lige...", "Hmm, lad mig finde noget..."]
        with st.spinner(random.choice(fillers)):
            references = []
            response_stream = requests.post("http://fakta-chat-1-0.mi-prod.svc.cloud.dbc.dk", json={"messages": st.session_state.messages}, stream=True)
