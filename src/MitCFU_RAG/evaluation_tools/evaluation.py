...
from fakta_chat.evaluation_tools.prompt_template import IN_CONTEXT_EXAMPLES, INSTRUCTIONS
from tqdm.auto import tqdm

logger = logging.getLogger(__name__)


def sample_evaluation_questions(limit : int = 30, question_type : str = 'all') -> list[dict]:
...
def attempt_api_call(client, model_name : str, messages : list[dict[str:str]], max_retries=10):
...
def generate_predictions(participant_model, evaluation_file):
    ...
    for line in tqdm(evaluation_file, desc="Generating Predictions"):
        ...
        reference, prediction = participant_model.evaluate(
            query
        )
...
def evaluate_responses(predictions : list, evaluation_model_name : str, openai_client, list_x : list, save_log : bool = True):
    n_correct = 0    
    ...
    for prediction_dict in tqdm(
        ...
        if response:
            ...
            if eval_res == 1:
                n_correct += 1
                n_correct_q_type[question_type] = n_correct_q_type.get(question_type, 0)+1
                correct = {'query': query, 'prediction': prediction,'ground_truth': ground_truth, 'response' : response, 'links' : links,
                           'links_in_prediction' : links_in_prediction}
                list_correct.append(correct)

                
            else:
                n_hallucination_q_type[question_type] = n_hallucination_q_type.get(question_type, 0)+1
                h = {'query': query, 'prediction': prediction,'ground_truth': ground_truth, 'response' : response, 'links' : links,
                     'links_in_prediction' : links_in_prediction}
    ...
    q_type_list = ['aggregation', 'comparison', 'false_premise', 'multi-hop', 'post_processing', 'set', 'simple']
        
    q_type_dict = {}
    for type in q_type_list:
        q_type_dict[type] = {'n hallucination' : n_hallucination_q_type.get(type, 0),
                             'n correct:': n_correct_q_type.get(type, 0)}
    
    
    
    dict_evaluation = {'list_hallucination': list_hallucination, 'list_correct': list_correct, 
...
def main(limit : int, question_type : str, compare_model : bool, save_log : bool):
    ...
    evaluation_results, df_results, df_question_type, dict_to_print = evaluate_responses(
    ...
    if compare_model:
        ...
        evaluation_results_compare, df_results_compare, df_question_type_compare, dict_to_print_compare = evaluate_responses(
...
def cli():
    ...
    parser.add_argument('-n', '--n-questions', default=30,
                        help='number of evaluation_questions')
    parser.add_argument('-q', '--question-type', default='all',
                        help='Type of question.')
    parser.add_argument('-c', '--compare-model', default=False, action='store_true',
                        help='set to True to compare two models')
    parser.add_argument('-s', '--save-log', default=True, action='store_false',
